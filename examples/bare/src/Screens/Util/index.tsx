...
import { generateFilePath, getRealPath, getVideoMetaData, getFileSize, download } from 'react-native-compressor';
...
const Index = () => {
      ...
      ImagePicker.launchImageLibrary(
        ...
        async (result: ImagePicker.ImagePickerResponse) => {
          if (result.didCancel) {
            ...
            if (result.assets) {
              const source: ImagePicker.Asset | undefined = result.assets[0];
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.container}>
        <Button onPress={generateTempFilePath} title="Generate Temp File Path" />
        <Button onPress={onGetRealPathIOS} title="Get Real Path (IOS) ph://" />
        <Button onPress={onGetRealPathAndroid} title="Get Real Path (Android) content://" />
