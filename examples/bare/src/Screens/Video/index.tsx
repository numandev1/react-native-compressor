...
import { View, Text, Button, Image, Alert, Platform, BackHandler, StyleSheet } from 'react-native';
import {
  Video,
  getRealPath,
  backgroundUpload,
...
export default function App() {
  ...
  const [compressedVideoThumbnail, setcompressedVideoThumbnail] = useState<string>();
  ...
  const testCompress = async () => {
    ...
    try {
      const dstUrl = await Video.compress(
        ...
        {
          progressDivider: 10,
          getCancellationId: (cancellationId) => (cancellationIdRef.current = cancellationId),
        },
        (progress) => {
  ...
  const onPressRemoteVideo = async () => {
    ...
    setSourceVideoThumbnail('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQvFTncZP4wzhJ9qH0dR1ZCCde_riH3aOoaVQZnOeVDnA&s');
    try {
      const dstUrl = await Video.compress(
        url,
        {
  ...
  const uploadByPostRequest = async (type: 'actual' | 'compressed' = 'actual') => {
    ...
    try {
      ...
      const result = await backgroundUpload(
        ...
        localFileUrl,
        {
          ...
          headers,
          getCancellationId: (cancellationId) => (cancellationIdForUploadRef.current = cancellationId),
        },
        (written, total) => {
          progressRef.current?.setProgress(written / total);
          console.log(written, total);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.thumbnailRow}>
        <View style={styles.sourceColumn}>
          {sourceVideoThumbnail && (
            <View>
              <Text>Source</Text>
              <Image style={styles.thumbnail} source={{ uri: sourceVideoThumbnail }} resizeMode="contain" />
              {sourceSize && <Text>Size: {sourceSize}</Text>}
              <Button title="Upload(Post)" onPress={() => uploadByPostRequest('actual')} />
              <Button title="Upload(Put)" onPress={() => uploadByPutRequest('actual')} />

              <Button title="Cancel Upload" onPress={() => cancelUploader()} />
            </View>
          )}
        </View>
        <View style={styles.compressedColumn}>
          {compressedVideoThumbnail && (
            <View>
              <Text>Compressed</Text>
              <Image style={styles.thumbnail} source={{ uri: compressedVideoThumbnail }} resizeMode="contain" />
              {compressedSize && <Text>Size: {compressedSize}</Text>}
              <Button title="Upload(Post)" onPress={() => uploadByPostRequest('compressed')} />
              <Button title="Upload(Put)" onPress={() => uploadByPutRequest('compressed')} />
            </View>
          )}
        </View>
      </View>
      <View style={styles.controlsRow}>
        ...
        <Button title="Compress" disabled={!sourceVideo} onPress={testCompress} />
      </View>
      <View style={styles.bottomSection}>
        <Button title="Remote Video (http://) and Compress" onPress={onPressRemoteVideo} />
        ...
        <View style={styles.backgroundSection}>
          {Platform.OS === 'ios' && <Button title={'Compress video from camera roll (ph://)'} onPress={onCompressVideofromCameraoll} />}
          <Button
          ...
          <Button
            title={backgroundMode ? 'Deactivate Background Mode' : 'Activate Background Mode'}
            onPress={() => {
              if (backgroundMode) {
                setBackgroundMode(false);
                Video.deactivateBackgroundTask()
                  .then((id: any) => console.log('Background Mode Deactivated', id))
                  .catch((error) => console.log('Failed to deactivate background task', error));
              } else {
                ...
                Video.activateBackgroundTask((data) => {
                  ...
                  .then((id: any) => console.log('Background Mode Activated', id))
                  .catch((error) => console.log('Failed to activate background task', error));
              }
            }}
          />
        </View>
